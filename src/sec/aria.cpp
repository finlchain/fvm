...
int32_t aria_enc(uint8_t *p_mk, uint32_t mk_len, uint32_t Nr, uint8_t *p_plaintext, uint32_t plaintext_len, uint8_t *p_ciphertext, uint32_t *p_ciphertext_len)
{
    uint8_t rk[16 * 17] = { 0, };
    uint8_t *p_src;
    uint32_t step = plaintext_len / ARIA_BUFFER_SIZE;
    uint8_t dummy_len;

    step++;

    *p_ciphertext_len = step * ARIA_BUFFER_SIZE;
    p_src = (uint8_t *)MALLOC_M(*p_ciphertext_len);

    dummy_len = *p_ciphertext_len - plaintext_len;
    MEMSET_M(p_src, dummy_len, *p_ciphertext_len);
    MEMCPY_M(p_src, p_plaintext, plaintext_len);

    for (uint32_t i = 0; i < step; i++)
    {
        EncKeySetup(p_mk, rk, mk_len * 8);
        Crypt(&p_src[i * ARIA_BUFFER_SIZE], Nr, rk, &p_ciphertext[i * ARIA_BUFFER_SIZE]);
    }
...
int32_t aria_dec(uint8_t *p_mk, uint32_t mk_len, uint32_t Nr, uint8_t *p_ciphertext, uint32_t ciphertext_len, uint8_t *p_plaintext, uint32_t *p_plaintext_len)
{
    uint8_t rk[16 * 17] = { 0, };
    uint32_t step = ciphertext_len / ARIA_BUFFER_SIZE;
    uint32_t idx;

    if (ciphertext_len % ARIA_BUFFER_SIZE)
    {
        return (ERROR_);
    }
    ...
    {
        DecKeySetup(p_mk, rk, mk_len * 8);
        Crypt(&p_ciphertext[idx * ARIA_BUFFER_SIZE], Nr, rk, &p_plaintext[idx * ARIA_BUFFER_SIZE]);
    }

    //
    *p_plaintext_len = ciphertext_len - p_plaintext[ciphertext_len - 1];

    return (SUCCESS_);
}

int32_t aria_enc_ori(uint8_t *p_mk, uint32_t mk_len, uint32_t Nr, uint8_t *p_plaintext, uint32_t plaintext_len, uint8_t *p_ciphertext, uint32_t *p_ciphertext_len)
{
    uint8_t rk[16 * 17] = { 0, };
    uint8_t *p_src;
    uint32_t step = plaintext_len / ARIA_BUFFER_SIZE;

    if (plaintext_len % ARIA_BUFFER_SIZE)
    {
        step++;
    }
...
int32_t aria_dec_ori(uint8_t *p_mk, uint32_t mk_len, uint32_t Nr, uint8_t *p_ciphertext, uint32_t ciphertext_len, uint8_t *p_plaintext, uint32_t *p_plaintext_len)
{
    uint8_t rk[16 * 17] = { 0, };
    uint32_t step = ciphertext_len / ARIA_BUFFER_SIZE;
    uint32_t idx;
    ...
    for(cnt = 0; cnt < ARIA_BUFFER_SIZE; cnt++)
    {
        int32_t idx = ciphertext_len-cnt-1;
        if (p_plaintext[idx] != 0x00)
        {
            break;
        }
    }
    ...
    //
    *p_plaintext_len = ciphertext_len - cnt;
...
for 14-round ARIA.\n");
    ...
    uint32_t c_len = 16, p_len = 14;
    uint32_t mk_len = 32, Nr = 16;

    printf("BEGIN testing the roundtrip...\n");
    printf("plaintext : "); printBlock(p); printf("\n");
    aria_enc(mk, mk_len, Nr, p, p_len, c, &c_len);
    printf("ciphertext: "); printBlock(c); printf("\n");
    memset(p, 0x01, p_len);
    aria_dec(mk, mk_len, Nr, c, c_len, p, &p_len);
